...
def get_batch(split):
    ...
    if split == 'train':
        data = np.memmap(os.path.join(data_dir, 'train.bin'), dtype=meta_dtype or np.uint16, mode='r')
    else:
        data = np.memmap(os.path.join(data_dir, 'val.bin'), dtype=meta_dtype or np.uint16, mode='r')
