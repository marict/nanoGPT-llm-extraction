...
def test_safe_torch_save_retry(monkeypatch, tmp_path):
    ...
    def flaky_save(obj, path):  # noqa: D401
        calls["count"] += 1
        # Fail first time then succeed
        if calls["count"] == 1:
            raise RuntimeError("unexpected pos 10 vs 9")
        # emulate writing file
        Path(path).write_text("ok")

    monkeypatch.setattr(train, "torch", SimpleNamespace(save=flaky_save))
...
def test_safe_torch_save_failure(monkeypatch, tmp_path):
    def always_fail(obj, path):
        raise RuntimeError("disk full?")

    monkeypatch.setattr(train, "torch", SimpleNamespace(save=always_fail))
